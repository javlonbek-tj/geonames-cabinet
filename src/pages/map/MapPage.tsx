...
export default function MapPage() {
  ...
  const { data: districtObjects, isFetching: objectsFetching } =
    useMapDistrictObjects(selectedDistrictId);
  ...
  const featureCounts = {
    ...
    districts:
      selectedRegionId !== null && selectedDistrictId === null
        ...
        : 0,
    mfy:
      districtObjects?.features.filter((f) => f.properties.isMfy).length ?? 0,
    streets:
      districtObjects?.features.filter((f) => !f.properties.isMfy).length ?? 0,
  };
