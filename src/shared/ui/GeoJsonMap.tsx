...
export default function GeoJsonMap({
  ...
  useEffect(() => {
    ...
    const geoLayer = L.geoJSON(geojson as GeoJSON.GeoJsonObject, {
      ...
      onEachFeature: (feature, layer) => {
        ...
        type LayerWithGeo = L.Layer & {
          getBounds?: () => L.LatLngBounds;
          getLatLng?: () => L.LatLng;
        };
        const l = layer as LayerWithGeo;
        const center = l.getBounds?.()?.isValid?.()
    ...
    paths.forEach((layer, i) => {
      layer.setStyle(i === highlightedIndexRef.current ? HIGHLIGHT_STYLE : DEFAULT_STYLE);
    });
  ...
  useEffect(() => {
    ...
    layersRef.current.forEach((layer, i) => {
      layer.setStyle(i === highlightedIndex ? HIGHLIGHT_STYLE : DEFAULT_STYLE);
    });
